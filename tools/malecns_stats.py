...
def main() -> int:
    ...
    # ---- generate KC taxonomy header (Allen BICCN principle: type-differentiated
    #      populations, fractions from THIS real dataset) ----
    kc_all = df[df["class"] == "Kenyon_Cell"]
    t = kc_all["type"].astype(str)
    n_ab = int(t.str.startswith("KCab").sum())          # alpha/beta
    n_abp = int(t.str.startswith("KCa'b'").sum())       # alpha'/beta'
    n_g = int(t.str.startswith("KCg").sum())            # gamma
    tot = max(n_ab + n_abp + n_g, 1)
    frac = [n_ab / tot, n_abp / tot, n_g / tot]
    # dynamics mapping is OUR design (documented): alpha-beta = long trace (LTM),
    # alpha'-beta' = short trace + fast learning (STM), gamma = intermediate.
    taus = [2.5, 0.3, 0.8]
    etas = [1.0, 1.5, 1.0]
    hdr = f"""// GENERATED by tools/malecns_stats.py from MaleCNS v1.0 real data — DO NOT EDIT.
// Source: Janelia FlyEM MaleCNS v1.0 (CC-BY), class == Kenyon_Cell, N={tot}
...
inline constexpr KcSubtypeSpec KC_SUBTYPES[3] = {{
    {{"alpha-beta",   {frac[0]:.4f}f, {taus[0]:.1f}f, {etas[0]:.1f}f}},
    {{"alpha-prime-beta-prime", {frac[1]:.4f}f, {taus[1]:.1f}f, {etas[1]:.1f}f}},
    {{"gamma",        {frac[2]:.4f}f, {taus[2]:.1f}f, {etas[2]:.1f}f}},
}};
inline constexpr int KC_N_SUBTYPES = 3;
}}  // namespace malefly
"""
    with open("src/circuits/kc_taxonomy.gen.h", "w") as f:
        f.write(hdr)
    lines.append("\n## KC 亜型 → 実装マッピング(自動生成 kc_taxonomy.gen.h)\n")
    lines.append("| 亜型 | 実測数 | 実測比率 | tau_elig[s] | eta_scale |")
    lines.append("|---|---|---|---|---|")
    for name, n, fr, ta, et in zip(["alpha-beta", "alpha'-beta'", "gamma"],
                                   [n_ab, n_abp, n_g], frac, taus, etas):
        lines.append(f"| {name} | {n:,} | {fr:.3f} | {ta} | {et} |")

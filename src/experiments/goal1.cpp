...
//
// All numbers in the output are measured from the actual run.
// ============================================================================
...
int run_goal1(const Goal1Config& cfg) {
    ...
    // ================= phase helpers =================
    u64 sim_ms = 0;
    u64 pn_spikes_in_odor = 0;
    u64 kc_spikes_in_odor = 0;
    u32 odor_windows = 0;
    ...
    auto probe_all = [&](std::vector<ProbeRow>& out) {
        // outcome-free shuffled presentations: DAN silent -> no plasticity
        ...
        for (u32 idx : order) {
            ...
            out.push_back({o.name, d.p_approach, d.valence, r.kc_active_frac});
    ...
    probe_all(probe_pre);  // naive baseline
    ...
    for (u32 trial = 0; trial < cfg.n_train_trials; ++trial) {
        ...
        run_gap(cfg.t_outcome_ms);                     // odor -> DAN delay
        if (rew != 0.0f) mb.apply_reinforcement(rew);  // DAN reinforcement
        run_gap(cfg.t_iti_ms);                         // inter-trial interval
    ...
    for (u32 i = 0; i < np; ++i) {
        present(*probe_list[i]);
        pats[i] = mb.trial_pattern();
        run_gap(cfg.t_iti_ms);
    }

    // per-pattern learned weight means (what the MB "knows" about each odor)
    ...
    // transfer-vs-overlap: does p_post(X) track circuit overlap with A (+) and B (-)?
    ...
    const u64 syn_ops = mb.pn2kc().ops + al.pool_ops() + mb.mbon_ops();
    const u64 mem_bytes = al.memory_bytes() + mb.memory_bytes();
    const f64 sim_s = static_cast<f64>(sim_ms) * 0.001;

    // ================= run hash (reproducibility fingerprint) =================
    ...
    ensure_parent_dir(cfg.out_json);
    {
        ...
        j.kv("concentration", cfg.concentration);
        ...
        j.kv("innate_bias", mbc.innate_bias);
        j.end_obj();
        ...
        j.kv("kc_spikes_per_presentation",
             odor_windows ? static_cast<f64>(kc_spikes_in_odor) /
                                static_cast<f64>(odor_windows)
                          : 0.0);
        j.end_obj();
    ...
    std::printf("seed=%llu trials=%u outcomes(+/-)=%u/%u\n",
                static_cast<unsigned long long>(cfg.seed), cfg.n_train_trials,
                n_eff_reward, n_eff_punish);
    std::printf("KC active frac: A=%.1f%% B=%.1f%% N=%.1f%% | PN Hz(odor)=%.1f | KC spikes/pres=%.2f\n",
                100.0 * aggregate(probe_post, "A_rewarded").kc,
                100.0 * aggregate(probe_post, "B_punished").kc,
                100.0 * aggregate(probe_post, "N_neutral").kc,
                static_cast<f64>(pn_spikes_in_odor) /

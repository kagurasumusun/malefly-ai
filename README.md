...
| 2 | 記憶(保持/干渉/遅延選択) | **達成** | 120s無強化後も A=0.91/B=0.25 維持、E+干渉下でも維持、**trace記憶は D≤1s 有効・2sで消滅**(τ=0.6s整合)、ソフトウェア記憶との差別化も実測 |
| 3 | 未知環境への適応(逆転学習、再訓練禁止) | 設計済 | ROADMAP §Goal 3 |
| 4 | 感覚→判断→行動統合(CX 導入) | 設計済 | ROADMAP §Goal 4 |
| E | 感情・自己・自我(構造から) | E1実装・E2–E4設計 | ROADMAP §E(定義→構造→実験→判定を先置き) |

単体テスト: **14,163 checks, 0 failures**。同 seed は bit 単位で再現(run_hash 検証)。
